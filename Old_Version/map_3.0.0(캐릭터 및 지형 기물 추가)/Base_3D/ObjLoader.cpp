...
void ObjLoader::LoadObjFileData(std::vector<ObjFileGroup*>& group, const char* fileName, bool isblend)
{
	...
	if ( fp != nullptr )
	{
		...
		while ( !feof(fp) )
		{
			...
			//else if ( tempBuffer[0] == 35 && tempBuffer[2] == 'V'&& tempBuffer[3] == 'e')
			//{
			//	char verticesSizeStr[256];
			//	int verticesSize = 0;;
			//	sscanf_s(tempBuffer, "%*s %*s %s", verticesSizeStr, 1024);
			//	verticesSize = (int)atoi(verticesSizeStr);
			//	vertex.reserve(verticesSize);
			//}
