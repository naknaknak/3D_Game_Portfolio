...
void GameState_HeightMap::Initialize( )
{
	...
	monster = new Minion( );
	monster->SetInitialPosition(D3DXVECTOR3(285, 11, -131));
	...
	monster = nullptr;
	monster = new Minion( );
	monster->SetInitialPosition(D3DXVECTOR3(285, 11, -151));
	monster->Initialize("Goat/", "Goat.x");
	...
	monster = nullptr;
	monster = new Minion( );
	monster->SetInitialPosition(D3DXVECTOR3(272, 11, -157));
	monster->Initialize("Goat/", "Goat.x");
	monster->SetHeightMap(heightMap);
	monster->ChangeCharacterState(CharacterState::CHARACTER_IDLE);
	monster->SetPlayer(lilith);
	...
	monster = nullptr;
	monster = new Minion( );
	monster->SetInitialPosition(D3DXVECTOR3(290, 11, -171));
	monster->Initialize("Goat/", "Goat.x");
	monster->SetMonsters(monsters);
	monster->SetHeightMap(heightMap);
	monster->ChangeCharacterState(CharacterState::CHARACTER_IDLE);
...
/*	if (goat)
	...
	if (lilith)
	{
		...
		float mp = 100.f;
		statusBar->GetPlayerMP(mp, 100.0f);
	...
	//몬스터 HP
	monsterHP = monsters[monsterHPIndex]->GetHP();
	statusBar->GetMonsterHP(monsterHP, monsters[monsterHPIndex]->GetMaxHP());
	if (monsterHP != monsters[monsterHPIndex]->GetMaxHP( ) && monsters[monsterHPIndex]->GetIsDead( ) == false)
	{
		statusBar->SetShowMonsterHp(true);
	}
	else
	{
		statusBar->SetShowMonsterHp(false);
