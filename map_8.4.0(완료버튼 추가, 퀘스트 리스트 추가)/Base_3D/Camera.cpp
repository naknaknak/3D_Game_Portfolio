...
{
	fFov = D3DX_PI * 0.25f;
	fAspect = (float)RESOLUTION_X / (float)RESOLUTION_Y;
	fNear = 1.0f;
	fFar = 10000.0f;

	vecForward = lookAt - eyePosition;
	D3DXVec3Cross(&vecRight, &upVector, &vecForward);
	...
	D3DXMatrixPerspectiveFovLH(&matProjection, fFov, fAspect, fNear, fFar);
	GameManager::GetDevice()->SetTransform(D3DTS_PROJECTION, &matProjection);

	frustum.Initialize( );
	// Fast Frustum
	frustum.SetPerspective(fFov, fAspect, fNear, fFar);
	frustum.Build(eyePosition, vecForward, vecRight, upVector);
}
...
LRESULT Camera::CameraInputProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	break;
	case WM_MOUSEWHEEL:
	deltaDistance= -GET_WHEEL_DELTA_WPARAM(wParam) / 20.f;
