...
AseFrame* AseLoader::Load(const char* filename)
{
	char fullPath[1024];
...
void AseLoader::Process_MAP_DIFFUSE(OUT MaterialTexture* mt)
{
	...
	do
	{
		...
		else if (IsEqual(aseToken, ID_BITMAP))
		{
			...
			char* str = GetToken( );
			mt->pTexDiffuse = TextureManager::GetTexture(str);
			mt->pTexNormal= TextureManager::GetNormalMap(str);
		}
	} while (level > 0);
}
...
void AseLoader::Process_MESH(OUT AseFrame* frame)
{
	...
	do
	{
		...
		else if (IsEqual(aseToken, ID_MESH_VERTEX_LIST))
		{
			...
			++count_num;
			mapVertexList[count_num] = position;
		}
