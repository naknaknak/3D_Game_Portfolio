...
{
	for (int i = 0; i < CORNER_TYPE_NUM; ++i)
	{
...
int QuadTree::_IsInFrustum(D3DXVECTOR3 * pHeightMapVertex)
{
	...
	// 4개 모두 절두체 안에 있음
	if ((b[0] + b[1] + b[2] + b[3]) == CORNER_TYPE_NUM) return FRUSTUM_COMPLETELY_IN;
...
void QuadTree::_FrustumCull(D3DXVECTOR3 * pHeightMapVertex)
{
	...
	if (m_pChild[0]) m_pChild[0]->_FrustumCull(pHeightMapVertex);
	if (m_pChild[1]) m_pChild[1]->_FrustumCull(pHeightMapVertex);
	if (m_pChild[2]) m_pChild[2]->_FrustumCull(pHeightMapVertex);
	if (m_pChild[3]) m_pChild[3]->_FrustumCull(pHeightMapVertex);
...
void QuadTree::_BuildNeighborNode(QuadTree * pRoot, D3DXVECTOR3 * pHeightMapVertex, int cx)
{
	...
	if (m_pChild[0])
	{
		m_pChild[0]->_BuildNeighborNode(pRoot, pHeightMapVertex, cx);
		m_pChild[1]->_BuildNeighborNode(pRoot, pHeightMapVertex, cx);
		m_pChild[2]->_BuildNeighborNode(pRoot, pHeightMapVertex, cx);
		m_pChild[3]->_BuildNeighborNode(pRoot, pHeightMapVertex, cx);
	}
}
...
QuadTree * QuadTree::_FindNode(D3DXVECTOR3 * pHeightMapVertex, int _0, int _1, int _2, int _3)
{
	...
	if (m_pChild[0])
	{
		...
		SetRect(&rc, (int)pHeightMapVertex[m_pChild[0]->m_nCorner[CORNER_TL]].x,
			(int)pHeightMapVertex[m_pChild[0]->m_nCorner[CORNER_TL]].z,
			(int)pHeightMapVertex[m_pChild[0]->m_nCorner[CORNER_BR]].x,
			(int)pHeightMapVertex[m_pChild[0]->m_nCorner[CORNER_BR]].z);
		// pt값이 점유범위안에 있다면 자식노드로 들어간다.
		if (IsInRect(&rc, pt))
			return m_pChild[0]->_FindNode(pHeightMapVertex, _0, _1, _2, _3);

		SetRect(&rc, (int)pHeightMapVertex[m_pChild[1]->m_nCorner[CORNER_TL]].x,
			(int)pHeightMapVertex[m_pChild[1]->m_nCorner[CORNER_TL]].z,
			(int)pHeightMapVertex[m_pChild[1]->m_nCorner[CORNER_BR]].x,
			(int)pHeightMapVertex[m_pChild[1]->m_nCorner[CORNER_BR]].z);
		if (IsInRect(&rc, pt))
			return m_pChild[1]->_FindNode(pHeightMapVertex, _0, _1, _2, _3);

		SetRect(&rc, (int)pHeightMapVertex[m_pChild[2]->m_nCorner[CORNER_TL]].x,
			(int)pHeightMapVertex[m_pChild[2]->m_nCorner[CORNER_TL]].z,
			(int)pHeightMapVertex[m_pChild[2]->m_nCorner[CORNER_BR]].x,
			(int)pHeightMapVertex[m_pChild[2]->m_nCorner[CORNER_BR]].z);
		if (IsInRect(&rc, pt))
			return m_pChild[2]->_FindNode(pHeightMapVertex, _0, _1, _2, _3);

		SetRect(&rc, (int)pHeightMapVertex[m_pChild[3]->m_nCorner[CORNER_TL]].x,
			(int)pHeightMapVertex[m_pChild[3]->m_nCorner[CORNER_TL]].z,
			(int)pHeightMapVertex[m_pChild[3]->m_nCorner[CORNER_BR]].x,
			(int)pHeightMapVertex[m_pChild[3]->m_nCorner[CORNER_BR]].z);
		if (IsInRect(&rc, pt))
